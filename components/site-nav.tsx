...
export function SiteNav({ brands }: { brands: Brand[] }) {
  const { favouriteIds, toggleFavourite, clearFavourites } = useFavourites();
  ...
  useEffect(() => {
    ...
    const handleClick = (event: MouseEvent | TouchEvent) => {
      ...
      if (!clickedFaves && isFavouritesOpen) setIsFavouritesOpen(false);
      if (!clickedMenu && isMobileMenuOpen) setIsMobileMenuOpen(false);
    };
      ...
      <nav className="sticky top-0 z-30 border-b border-[#f2f2f2] bg-[#fbfbfb]/90 backdrop-blur">
        <div className="relative mx-auto flex w-full max-w-6xl items-center justify-between px-4 py-3 sm:px-8 sm:py-4">
          ...
          <div className="hidden items-center gap-6 text-sm font-medium text-zinc-700 sm:flex">
            <a className="hover:text-zinc-900" href="/#brands">Brands</a>
            <a className="hover:text-zinc-900" href="/#filters">Filters</a>
          ...
          {isMobileMenuOpen ? (
            ...
            >
              <div className="flex flex-col gap-2 text-base font-medium text-[#3a3a3a]">
                <a className="rounded-lg px-2 py-2 hover:bg-zinc-50" href="/#brands">Brands</a>
                <a className="rounded-lg px-2 py-2 hover:bg-zinc-50" href="/#filters">Filters</a>
                <a className="rounded-lg px-2 py-2 hover:bg-zinc-50" href="/#about">About</a>
              </div>
            </div>
          ) : null}
        </div>
